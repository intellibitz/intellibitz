...
public class MsgsGrpPeopleEmailsContentProvider extends
        ContentProvider {

    public static final String TAG = "MsgsGrpPeopleEmailsCP";
    public static final String TABLE_MSGSGRPPEOPLEEMAILS = "msgsgrppeopleemails";
    public static final String CREATE_TABLE_MSGSGRPPEOPLEEMAILS = "CREATE TABLE "
            + TABLE_MSGSGRPPEOPLEEMAILS + MessageItemColumns.MESSAGECHAT_SCHEMA;
    ...
    public static final String AUTHORITY =
            "intellibitz.intellidroid.content.MsgsGrpPeopleEmailsContentProvider";
    public static final Uri CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + TABLE_MSGSGRPPEOPLEEMAILS);
    public static final Uri JOIN_CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + "join_" + TABLE_MSGSGRPPEOPLEEMAILS);
    //    to execute raw sql.. example select count(*)
    public static final Uri RAW_CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + "raw_" + TABLE_MSGSGRPPEOPLEEMAILS);
    // UriMatcher stuff
    private static final int MSGSGRPPEOPLEEMAILS_DIR_TYPE = 1;
    private static final int MSGSGRPPEOPLEEMAILS_JOIN_DIR_TYPE = 2;
    private static final int MSGSGRPPEOPLEEMAILS_ITEM_TYPE = 3;
    private static final int MSGSGRPPEOPLEEMAILS_DATA_ITEM_TYPE = 4;
    private static final int MSGSGRPPEOPLEEMAILS_JOIN_ITEM_TYPE = 5;
    private static final int MSGSGRPPEOPLEEMAILS_JOIN_DATA_ITEM_TYPE = 6;
    private static final int MSGSGRPPEOPLEEMAILS_RAW_DIR_TYPE = 7;
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        // to get definitions...
        matcher.addURI(AUTHORITY, TABLE_MSGSGRPPEOPLEEMAILS,
                MSGSGRPPEOPLEEMAILS_DIR_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGSGRPPEOPLEEMAILS,
                MSGSGRPPEOPLEEMAILS_JOIN_DIR_TYPE);
        matcher.addURI(AUTHORITY, TABLE_MSGSGRPPEOPLEEMAILS +
                "/#", MSGSGRPPEOPLEEMAILS_ITEM_TYPE);
        matcher.addURI(AUTHORITY, TABLE_MSGSGRPPEOPLEEMAILS +
                "/*", MSGSGRPPEOPLEEMAILS_DATA_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGSGRPPEOPLEEMAILS +
                        "/#", MSGSGRPPEOPLEEMAILS_JOIN_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGSGRPPEOPLEEMAILS +
                        "/*", MSGSGRPPEOPLEEMAILS_JOIN_DATA_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "raw_" + TABLE_MSGSGRPPEOPLEEMAILS, MSGSGRPPEOPLEEMAILS_RAW_DIR_TYPE);
        // to get suggestions...
...
//        if (!bcc.isEmpty())
    ...
    public static long createOrUpdateMsgsGrpPeopleEmails(
...
*/
    ...
    public static void savesMsgsGrpPeopleChats(
        ...
        if (0 == did) {
            cursor = databaseHelper.query(db, TABLE_MSGSGRPPEOPLEEMAILS,
            ...
            cursor = databaseHelper.query(db, MsgsGrpPeopleEmailsContentProvider.TABLE_MSGSGRPPEOPLEEMAILS,
                    new String[]{MessageItemColumns.KEY_ID},
                    colName + " = ?",
                    new String[]{devId}, null);
        }
        if (null == cursor || 0 == cursor.getCount()) {
            ...
            databaseHelper.insert(db, MsgsGrpPeopleEmailsContentProvider.TABLE_MSGSGRPPEOPLEEMAILS, null, values);
        } else {
            cursor.close();
            databaseHelper.update(db, MsgsGrpPeopleEmailsContentProvider.TABLE_MSGSGRPPEOPLEEMAILS, values,
                    colName + " = ?",
                    new String[]{devId});
        }
    }
...
//                MainApplicationSingleton.getDateTimeMillis(messageItem.getTimestamp()));
    ...
    public String getType(@NonNull Uri uri) {
        switch (URI_MATCHER.match(uri)) {
            case MSGSGRPPEOPLEEMAILS_DIR_TYPE:
                return MSGSGRPPEOPLECHATS_DIR_MIME_TYPE;
            case MSGSGRPPEOPLEEMAILS_JOIN_DIR_TYPE:
                return MSGSGRPPEOPLECHATS_JOIN_DIR_MIME_TYPE;
            case MSGSGRPPEOPLEEMAILS_ITEM_TYPE:
                return MSGSGRPPEOPLECHATS_ITEM_MIME_TYPE;
            case MSGSGRPPEOPLEEMAILS_DATA_ITEM_TYPE:
                return MSGSGRPPEOPLECHATS_DATA_ITEM_MIME_TYPE;
            case MSGSGRPPEOPLEEMAILS_JOIN_ITEM_TYPE:
                return MSGSGRPPEOPLECHATS_JOIN_ITEM_MIME_TYPE;
            case MSGSGRPPEOPLEEMAILS_JOIN_DATA_ITEM_TYPE:
                return MSGSGRPPEOPLECHATS_JOIN_DATA_ITEM_MIME_TYPE;
            case MSGSGRPPEOPLEEMAILS_RAW_DIR_TYPE:
...
//        String id = uri.getLastPathSegment();
        switch (URI_MATCHER.match(uri)) {
            ...
            case MSGSGRPPEOPLEEMAILS_JOIN_DATA_ITEM_TYPE:
                try {
                    cursor = databaseHelper.query(TABLE_MSGSGRPPEOPLEEMAILS,
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case MSGSGRPPEOPLEEMAILS_JOIN_DATA_ITEM_TYPE:
                try {
                    long id = databaseHelper.insert(TABLE_MSGSGRPPEOPLEEMAILS, null, values);
    ...
    public int update(@NonNull Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case MSGSGRPPEOPLEEMAILS_JOIN_DATA_ITEM_TYPE:
                try {
                    id = databaseHelper.update(TABLE_MSGSGRPPEOPLEEMAILS, values, where, whereArgs);
                    updateUri = ContentUris.withAppendedId(
                            MsgsGrpPeopleEmailsContentProvider.CONTENT_URI, id);
                    Context context = getContext();
                    if (null != context) {
            ...
            case MSGSGRPPEOPLEEMAILS_JOIN_DATA_ITEM_TYPE:
                try {
                    id = databaseHelper.delete(
                            TABLE_MSGSGRPPEOPLEEMAILS, where, whereArgs);
