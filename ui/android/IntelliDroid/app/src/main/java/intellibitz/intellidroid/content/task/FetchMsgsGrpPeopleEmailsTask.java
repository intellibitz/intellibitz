...
/**
 */
public class FetchMsgsGrpPeopleEmailsTask extends
        IntellibitzAsyncTask {

    private static final String TAG = "MsgsGrpPeopleEmailsTask";

    private FetchMsgsGrpPeopleTaskListener fetchMsgsGrpPeopleTaskListener;
    private String filter;
    private ArrayList<MessageItem> messageItems;

    public FetchMsgsGrpPeopleEmailsTask(String filter, Context context) {
        super(context);
        this.filter = filter;
    }
...
//                    MsgsGrpPeopleContentProvider.JOIN_CONTENT_URI,
                MsgsGrpPeopleEmailsContentProvider.CONTENT_URI,
                null,
                selection, selArgs,
...
*/
        ...
        if (cursor != null && cursor.getCount() > 0) {
            messageItems = MsgsGrpPeopleEmailsContentProvider.fillMessageThreadItemsFromCursor(cursor);
