...
import static intellibitz.intellidroid.content.MsgChatContactContentProvider.TABLE_MSGCHATCONTACT;
import static intellibitz.intellidroid.db.MessagesContactsJoinColumns.KEY_MSGTHREAD_ID;

/**
 *
 */
public class MsgChatContactsContentProvider extends
        ContentProvider {

    public static final String TAG = "MsgChatContactsCP";
    public static final String TABLE_MSGCHATCONTACTS = "msgchatcontacts";
    public static final String TABLE_MSGCHATCONTACTS_CONTACT_JOIN = "msgchatcontacts_contact";
    public static final String CREATE_TABLE_MSGCHATCONTACTS = "CREATE TABLE "
            + TABLE_MSGCHATCONTACTS + ContactItemColumns.TABLE_CONTACTS_SCHEMA;
    ...
    public static final String CREATE_TABLE_MSGCHATCONTACTS_CONTACT_JOIN = "CREATE TABLE "
            + TABLE_MSGCHATCONTACTS_CONTACT_JOIN + "( " +
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        matcher.addURI(AUTHORITY, TABLE_MSGCHATCONTACTS +
                "/#", CONTACTS_ITEM_TYPE);
        matcher.addURI(AUTHORITY, TABLE_MSGCHATCONTACTS +
                "/*", CONTACTS_DATA_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGCHATCONTACTS, CONTACTS_JOIN_DIR_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGCHATCONTACTS +
                        "/#", CONTACTS_JOIN_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGCHATCONTACTS +
                        "/*", CONTACTS_JOIN_DATA_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "raw_" + TABLE_MSGCHATCONTACTS, CONTACTS_RAW_DIR_TYPE);
        // to get suggestions...
...
*/
    ...
    public static List<ContactItem> createsContactsFromCursor(Cursor cursor) {
...
//                        " ak.intellibitz_id as akonid, ak.status as akstatus, " +
                        ...
                        "  FROM  " +
                        TABLE_MSGCHATCONTACTS + " gc " +
                        "left outer join " + TABLE_MSGCHATCONTACTS_CONTACT_JOIN +
...
//        Cursor cursor = getAKContactCursorJoin(db, item, id);
        Cursor cursor = databaseHelper.query(db, MsgChatContactContentProvider.TABLE_MSGCHATCONTACT,
//                projection
...
//        creates the join
//        // TODO: 01-07-2016
        MsgChatContactContentProvider.createOrUpdateContactIntellibitzContactByContactJoin(databaseHelper, db, item, item.get_id());
        ContentValues values = new ContentValues();
//        message thread is the parent, email item child
        MsgChatContactContentProvider.fillContentValuesFromContactItem(item, values);
        databaseHelper.update(db, MsgChatContactContentProvider.TABLE_MSGCHATCONTACT, values,
                ContactItemColumns.KEY_ID + " = ?",
...
//                selection args
        ...
        if (null == cursor || 0 == cursor.getCount()) {
            ...
            item.set_id(_id);
//            updateMessageThreadEmail(item, id);
            databaseHelper.update(db, MsgChatContactContentProvider.TABLE_MSGCHATCONTACT, values, ContactItemColumns.KEY_ID + " = ?",
                    new String[]{String.valueOf(_id)});
        }
//        creates the join
        createOrUpdateContactsContactJoin(databaseHelper, db, _id, id);
...
//            deletes the item
            _id = databaseHelper.delete(db, MsgChatContactContentProvider.TABLE_MSGCHATCONTACT,
                    ContactItemColumns.KEY_ID + " = ? ",
...
//        deletes the join
        ...
        if (0 == row) return 0;
//            deletes the item
        return databaseHelper.delete(db, MsgChatContactContentProvider.TABLE_MSGCHATCONTACT,
                ContactItemColumns.KEY_ID + " = ? ",
                new String[]{String.valueOf(_id)});
    }
...
//        checks if GroupContact already exists
        Cursor cursor = databaseHelper.query(db, MsgChatContactsContentProvider.TABLE_MSGCHATCONTACTS,
//                projection
                new String[]{ContactItemColumns.KEY_ID},
//                selection
                ContactItemColumns.KEY_DATA_ID + " = ? ",
...
//        does not exist.. insert
        if (null == cursor || 0 == cursor.getCount()) {
            if (cursor != null) cursor.close();
            long _id = databaseHelper.insert(db, MsgChatContactsContentProvider.TABLE_MSGCHATCONTACTS, null,
...
//        for a group, creates a intellibitz contact.. from the contacts direct (no need for child)
        MsgChatContactContentProvider.createOrUpdateContactIntellibitzContactByContactJoin(
                databaseHelper, db, contactItem, contactItem.get_id());
        databaseHelper.update(db, MsgChatContactsContentProvider.TABLE_MSGCHATCONTACTS,
                MsgChatContactsContentProvider.fillContentValuesFromContactThreadItem(
                        contactItem, new ContentValues()),
                ContactItemColumns.KEY_ID + " = ?",
    ...
    public static long getContactThreadContactJoin(
        ...
        long _id = 0;
        Cursor c = databaseHelper.query(db, TABLE_MSGCHATCONTACTS_CONTACT_JOIN,
...
//        checks if GroupContact already exists
/*
        Cursor cursor = query(db, ContactsContentProvider.TABLE_MSGCHATCONTACTS,
//                projection
...
//                selection args
        ...
        if (null == cursor || 0 == cursor.getCount()) {
            cursor = query(db, ContactsContentProvider.TABLE_MSGCHATCONTACTS,
                    new String[]{ContactsContentProvider.ContactItemColumns.KEY_ID},
...
//        does not exist.. insert
        if (null == cursor || 0 == cursor.getCount()) {
            if (cursor != null) cursor.close();
            long _id = insert(db, ContactsContentProvider.TABLE_MSGCHATCONTACTS, null,
...
//            exists.. update
            ...
            contactItem.set_id(_id);
            cursor.close();
            update(db, ContactsContentProvider.TABLE_MSGCHATCONTACTS,
                    ContactsContentProvider.fillContentValuesFromContactThreadItem(
                            contactItem, new ContentValues()),
                    ContactsContentProvider.ContactItemColumns.KEY_ID + " = ?",
                    new String[]{String.valueOf(contactItem.get_id())});
...
//        checks group contact, msg thread join first
        Cursor cursor = databaseHelper.query(db, TABLE_MSGCHATCONTACTS_CONTACT_JOIN,
...
//            throws sql execeptions here.. to break transaction
            cursor = databaseHelper.query(db, TABLE_MSGCHATCONTACTS,
...
//        checks group contact, msg thread join first
        Cursor cursor = databaseHelper.query(db,
                MessageChatContentProvider.TABLE_MESSAGESCHAT_CONTACTS_JOIN,
                new String[]{MessagesContactsJoinColumns.KEY_MSGTHREAD_ID},
        ...
        if (null == cursor || 0 == cursor.getCount()) {
            if (cursor != null) cursor.close();
//            throws sql execeptions here.. to break transaction
            cursor = databaseHelper.query(db, MessageChatContentProvider.TABLE_MESSAGESCHAT,
                    new String[]{MessageItemColumns.KEY_ID},
                    MessageItemColumns.KEY_DATA_ID + " = ?",
                    new String[]{String.valueOf(contactItem.getDataId())}, null
            );
...
//        if message thread exists, join
//        checks group contact, msg thread join first
        Cursor cursor = databaseHelper.query(db, MessageChatContentProvider.TABLE_MESSAGESCHAT_CONTACTS_JOIN,
                new String[]{MessagesContactsJoinColumns.KEY_MSGTHREAD_ID},
                ContactItemColumns.KEY_ID + " = ?",
                new String[]{String.valueOf(contactItem.get_id())}, null
        );
...
//        creates the join
    ...
    public static Cursor getMessageContactThreadCursorJoin(
        ...
        String selectQuery = "SELECT  * FROM " + MessageChatContentProvider.TABLE_MESSAGECHAT + " nt " +
                " left join " + MessageChatContentProvider.TABLE_MESSAGECHAT_CONTACTS_JOIN +
...
//        Log.e(TAG, selectQuery);
    ...
    public static long createOrUpdateMessagesContactsJoin(
        ...
        if (null == cursor || 0 == cursor.getCount()) {
            if (cursor != null) cursor.close();
            cursor = databaseHelper.query(MsgChatContactsContentProvider.TABLE_MSGCHATCONTACTS,
                    new String[]{ContactItemColumns.KEY_ID},
                    ContactItemColumns.KEY_DATA_ID + " = ? ",
                    new String[]{item.getDataId()}, null);
            if (null != cursor && 0 != cursor.getCount()) {
...
//        creates the join
    ...
    public static long createOrUpdateMessagesContactsJoin(
        ...
        if (0 == _id) {
            _id = databaseHelper.insert(db,
                    MessageChatContentProvider.CREATE_TABLE_MESSAGESCHAT_CONTACTS_JOIN, null, values);
        } else {
            _id = databaseHelper.update(db,
                    MessageChatContentProvider.TABLE_MESSAGESCHAT_CONTACTS_JOIN, values,
    ...
    public static Cursor getMessageThreadContactThreadCursorJoin(
            DatabaseHelper databaseHelper, ContactItem item, long id) {
        String[] args = new String[]{item.getDataId(), String.valueOf(id)};
        String selectQuery = "SELECT  * FROM " + MessageChatContentProvider.TABLE_MESSAGESCHAT + " nt " +
                " left join " + MessageChatContentProvider.TABLE_MESSAGESCHAT_CONTACTS_JOIN +
            ...
            case CONTACTS_DATA_ITEM_TYPE:
                try {
                    cursor = databaseHelper.query(
                            TABLE_MSGCHATCONTACTS, projection,
                            selection,
...
/*
                cursor = databaseHelper.query(DatabaseHelper.TABLE_MSGCHATCONTACTS,
                        projection,
            ...
            case CONTACTS_ITEM_TYPE:
                try {
                    ...
                    Uri insertUri = ContentUris.withAppendedId(
                            MsgChatContactsContentProvider.CONTENT_URI, ids[0]);
                    Context context = getContext();
                    if (null != context) {
...
//                    id = (int) databaseHelper.createOrUpdateMessagesContacts(item);
            ...
            case CONTACTS_JOIN_DATA_ITEM_TYPE:
                try {
                    int rows = databaseHelper.update(TABLE_MSGCHATCONTACTS,
                    ...
                    if (null != context) {
                        Uri updateUri = ContentUris.withAppendedId(
                                MsgChatContactsContentProvider.CONTENT_URI, rows);
                        context.getContentResolver().notifyChange(updateUri, null);
                    }
                    return rows;
                } catch (SQLException e) {
    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case CONTACTS_DATA_ITEM_TYPE:
                try {
                    int id = databaseHelper.delete(TABLE_MSGCHATCONTACTS,
                            selection, selectionArgs);
                    Uri insertUri = ContentUris.withAppendedId(MsgChatContactsContentProvider.CONTENT_URI, id);
                    Context context = getContext();
                    if (null != context) {
                        context.getContentResolver().notifyChange(insertUri, null);
                    }
            ...
            case CONTACTS_JOIN_DATA_ITEM_TYPE:
                try {
                    ...
                    Uri insertUri = ContentUris.withAppendedId(
                            MsgChatContactsContentProvider.CONTENT_URI, row);
                    Context context = getContext();
                    if (null != context) {
                        context.getContentResolver().notifyChange(insertUri, null);
                    }
