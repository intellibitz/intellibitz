 ...
 *
 */
public class MsgChatGrpContactContentProvider extends
        ContentProvider {

    public static final String TAG = "MsgChatGrpContactCP";
    public static final String TABLE_MSGCHATGRPCONTACT = "msgchatgrpcontact";
    public static final String TABLE_MSGCHATGRPCONTACT_INTELLIBITZCONTACTS_JOIN = "msgchatgrpcontact_intellibitzcontacts";
    public static final String CREATE_TABLE_MSGCHATGRPCONTACT = "CREATE TABLE "
            + TABLE_MSGCHATGRPCONTACT + ContactItemColumns.TABLE_CONTACTS_SCHEMA;
    ...
    public static final String CREATE_TABLE_MSGCHATGRPCONTACT_INTELLIBITZCONTACTS_JOIN = "CREATE TABLE "
            + TABLE_MSGCHATGRPCONTACT_INTELLIBITZCONTACTS_JOIN + "( " +
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        // to get definitions...
        matcher.addURI(AUTHORITY, TABLE_MSGCHATGRPCONTACT, CONTACT_DIR_TYPE);
        matcher.addURI(AUTHORITY, TABLE_MSGCHATGRPCONTACT +
                "/#", CONTACT_ITEM_TYPE);
        matcher.addURI(AUTHORITY, TABLE_MSGCHATGRPCONTACT +
                "/*", CONTACT_DATA_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGCHATGRPCONTACT, CONTACT_JOIN_DIR_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGCHATGRPCONTACT +
                        "/#", CONTACT_JOIN_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGCHATGRPCONTACT +
                        "/*", CONTACT_JOIN_DATA_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "raw_" + TABLE_MSGCHATGRPCONTACT, CONTACT_RAW_DIR_TYPE);
        // to get suggestions...
...
//                selection args
        ...
        if (null == cursor || 0 == cursor.getCount()) {
            ...
            String last = cursor.getString(cursor.getColumnIndex(
                    ContactItemColumns.KEY_LAST_NAME));
            String disp = cursor.getString(cursor.getColumnIndex(
                    ContactItemColumns.KEY_DISPLAY_NAME));
            cursor.close();
//            // TODO: 04-07-2016
...
//            the hack is for msg thread groups.. so it can get device contact from contact item
    ...
    public static Cursor getContactIntellibitzContactCursorJoin(DatabaseHelper databaseHelper,
        ...
        String selectQuery = "SELECT  * FROM " + MsgChatGrpContactContentProvider.TABLE_MSGCHATGRPCONTACT + " nt " +
                " left join " + TABLE_MSGCHATGRPCONTACT_INTELLIBITZCONTACTS_JOIN +
...
//        Log.e(TAG, selectQuery);
    ...
    public static long createOrUpdateContactIntellibitzContactJoin(
        ...
        if (0 == _id) {
            _id = databaseHelper.insert(db, TABLE_MSGCHATGRPCONTACT_INTELLIBITZCONTACTS_JOIN, null, values);
        } else {
            _id = databaseHelper.update(db,
                    TABLE_MSGCHATGRPCONTACT_INTELLIBITZCONTACTS_JOIN, values,
    ...
    public static long getContactIntellibitzContactJoin(
            DatabaseHelper databaseHelper, SQLiteDatabase db, long id, long fk) {
        long _id = 0;
        Cursor c = databaseHelper.query(db, TABLE_MSGCHATGRPCONTACT_INTELLIBITZCONTACTS_JOIN,
    ...
    public Cursor query(@NonNull Uri uri, String[] projection, String selection,
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case CONTACT_JOIN_DIR_TYPE:
                try {
                    cursor = databaseHelper.query(TABLE_MSGCHATGRPCONTACT,
                            projection,
            ...
            case CONTACT_ITEM_TYPE:
                try {
                    ...
                    Uri insertUri = ContentUris.withAppendedId(
                            MsgChatGrpContactContentProvider.CONTENT_URI, id);
                    Context context = getContext();
                    if (null != context) {
...
//                    context.getContentResolver().notifyChange(uri, null);
    ...
    public int update(@NonNull Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case CONTACT_ITEM_TYPE:
                try {
                    id = databaseHelper.update(TABLE_MSGCHATGRPCONTACT,
                            values,
                            selection, selectionArgs);
                    updateUri = ContentUris.withAppendedId(MsgChatGrpContactContentProvider.CONTENT_URI, id);
                    if (null != context) {
    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case CONTACT_ITEM_TYPE:
                try {
                    int id = databaseHelper.delete(TABLE_MSGCHATGRPCONTACT,
                            selection, selectionArgs);
                    Uri delUri = ContentUris.withAppendedId(MsgChatGrpContactContentProvider.CONTENT_URI, id);
                    Context context = getContext();
                    if (null != context) {
...
and mt._id = '11'
        ...
        String selectQuery = "SELECT  * FROM " + ContactsContentProvider.TABLE_MSGCHATGRPCONTACTS + " ct " +
                " left join " + TABLE_MSGCHATGRPCONTACTS_CONTACT_JOIN +
                " cte on ct.[_id] = cte.[" + DatabaseHelper.ContactsContactJoinColumns.KEY_CONTACTTHREAD_ID + "]  " +
                " left join " + ContactContentProvider.TABLE_MSGCHATGRPCONTACT + " em on cte.[" +
