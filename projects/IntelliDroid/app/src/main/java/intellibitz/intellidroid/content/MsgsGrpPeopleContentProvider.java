...
public class MsgsGrpPeopleContentProvider extends
        ContentProvider {

    public static final String TAG = "MsgsGrpPeopleCP";
    public static final String TABLE_MSGSGRPPEOPLE = "msgsgrppeople";
    public static final String CREATE_TABLE_MSGSGRPPEOPLE = "CREATE TABLE "
            + TABLE_MSGSGRPPEOPLE + MessageItemColumns.MESSAGECHAT_SCHEMA;
    ...
    public static final String AUTHORITY = "intellibitz.intellidroid.content.MsgsGrpPeopleContentProvider";
    public static final Uri CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + TABLE_MSGSGRPPEOPLE);
    public static final Uri JOIN_CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + "join_" + TABLE_MSGSGRPPEOPLE);
    //    to execute raw sql.. example select count(*)
    public static final Uri RAW_CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + "raw_" + TABLE_MSGSGRPPEOPLE);
    // UriMatcher stuff
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        // to get definitions...
        matcher.addURI(AUTHORITY, TABLE_MSGSGRPPEOPLE,
                MESSAGE_THREADS_DIR_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGSGRPPEOPLE,
        ...
        matcher.addURI(AUTHORITY,
                "raw_" + TABLE_MSGSGRPPEOPLE, MESSAGE_THREADS_RAW_DIR_TYPE);
        // to get suggestions...
    ...
    public static void fillsMessageItemFromCursor(Cursor cursor, MessageItem messageItem) {
        ...
        messageItem.setCloudPic(cursor.getString(cursor.getColumnIndex(
                MessageItemColumns.KEY_CLOUD_PIC)));
        ...
        messageItem.setDelivered(cursor.getInt(cursor.getColumnIndex(
                MessageItemColumns.KEY_IS_DELIVERED)));
        messageItem.setLocked(cursor.getInt(cursor.getColumnIndex(
                MessageItemColumns.KEY_IS_LOCKED)));
...
//        messageItems.addAll(set);
    ...
    public static int lockMsgs(String[] item, Context context) throws IOException {
        ContentValues contentValues = new ContentValues();
        contentValues.put(MessageItemColumns.KEY_IS_LOCKED, true);
        return context.getApplicationContext().getContentResolver().update(CONTENT_URI,
                contentValues, MessageItemColumns.KEY_DATA_ID + " = ? ", item);
    }

    public static int unlockMsgs(String[] item, Context context) throws IOException {
        ContentValues contentValues = new ContentValues();
        contentValues.put(MessageItemColumns.KEY_IS_LOCKED, false);
        return context.getApplicationContext().getContentResolver().update(CONTENT_URI,
                contentValues, MessageItemColumns.KEY_DATA_ID + " = ? ", item);
    }
...
//        if (!bcc.isEmpty())
    ...
    public static long createOrUpdateMsgsGrpPeople(
        ...
        MessageItem message = (MessageItem) messageItem.cloneShal();
        message.set_id(0);
//        chat and chat group
        savesMsgsGrpPeople(databaseHelper, db, message, contacts);
...
/*
        ...
        for (ContactItem item : items) {
            savesMsgsGrpPeople(databaseHelper, db, messages, item);
        }
*/
        return messageItem.get_id();
    }

    public static void savesMsgsGrpPeople(
            DatabaseHelper dbHelper, SQLiteDatabase db, MessageItem message, ContactItem contact) {
        long did = contact.getDeviceContactId();
        final String intellibitzId = contact.getIntellibitzId();
        final String deviceContactId = String.valueOf(did);
        String devId = intellibitzId;
        ContentValues values = new ContentValues();
        message.setDeviceContactId(did);
        message.setIntellibitzId(intellibitzId);
        message.setTypeId(contact.getTypeId());
        message.setDevice(contact.isDevice());
        message.setGroup(contact.isGroup());
        message.setEmailItem(contact.isEmailItem());
        message.setFirstName(contact.getFirstName());
        message.setLastName(contact.getLastName());
        message.setDisplayName(contact.getDisplayName());
        fillContentValuesFromMessageItem(message, values);
        Cursor cursor;
        String colName = MessageItemColumns.KEY_INTELLIBITZ_ID;
        if (0 == did) {
            cursor = dbHelper.query(db, TABLE_MSGSGRPPEOPLE,
            ...
            cursor = dbHelper.query(db, MsgsGrpPeopleContentProvider.TABLE_MSGSGRPPEOPLE,
                    new String[]{MessageItemColumns.KEY_ID},
                    colName + " = ?",
                    new String[]{devId}, null);
        }
        if (null == cursor || 0 == cursor.getCount()) {
            ...
            dbHelper.insert(db, MsgsGrpPeopleContentProvider.TABLE_MSGSGRPPEOPLE, null, values);
        } else {
            cursor.close();
            dbHelper.update(db, MsgsGrpPeopleContentProvider.TABLE_MSGSGRPPEOPLE, values,
                    colName + " = ?",
                    new String[]{devId});
        }
    }
    ...
    public static ContentValues fillContentValuesFromMessageItem(
        ...
        MainApplicationSingleton.fillIfNotNull(values,
                MessageItemColumns.KEY_CLOUD_PIC, messageItem.getCloudPic());
        MainApplicationSingleton.fillIfNotNull(values,
        ...
        MainApplicationSingleton.fillIfNotNull(values,
                MessageItemColumns.KEY_IS_DELIVERED, messageItem.isDelivered());
        MainApplicationSingleton.fillIfNotNull(values,
                MessageItemColumns.KEY_IS_LOCKED, messageItem.isLocked());
        MainApplicationSingleton.fillIfNotNull(values,
                MessageItemColumns.KEY_IS_GROUP, messageItem.isGroup());
        MainApplicationSingleton.fillIfNotNull(values,
                MessageItemColumns.KEY_IS_EMAIL, messageItem.isEmailItem());
...
//        String id = uri.getLastPathSegment();
        switch (URI_MATCHER.match(uri)) {
            ...
            case MESSAGE_THREADS_JOIN_DATA_ITEM_TYPE:
                try {
                    cursor = databaseHelper.query(TABLE_MSGSGRPPEOPLE,
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case MESSAGE_THREADS_JOIN_DATA_ITEM_TYPE:
                try {
                    long id = databaseHelper.insert(TABLE_MSGSGRPPEOPLE, null, values);
    ...
    public int update(@NonNull Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case MESSAGE_THREADS_JOIN_DATA_ITEM_TYPE:
                try {
                    id = databaseHelper.update(TABLE_MSGSGRPPEOPLE, values, where, whereArgs);
                    updateUri = ContentUris.withAppendedId(
                            MsgsGrpPeopleContentProvider.CONTENT_URI, id);
                    Context context = getContext();
                    if (null != context) {
            ...
            case MESSAGE_THREADS_JOIN_DATA_ITEM_TYPE:
                try {
                    id = databaseHelper.delete(
                            TABLE_MSGSGRPPEOPLE, where, whereArgs);
