...
public class MsgsGrpPeopleChatGroupsContentProvider extends
        ContentProvider {

    public static final String TAG = "PeopleChatGroupsCP";
    public static final String TABLE_MSGSGRPPEOPLECHATGROUPS = "msgsgrppeoplechatgroups";
    public static final String CREATE_TABLE_MSGSGRPPEOPLECHATGROUPS = "CREATE TABLE "
            + TABLE_MSGSGRPPEOPLECHATGROUPS + MessageItemColumns.MESSAGECHAT_SCHEMA;
    //    The content provider scheme
    public static final String SCHEME = "content://";
    // MIME types used for searching words or looking up a single definition
    public static final String MSGSGRPPEOPLECHATGROUPS_DIR_MIME_TYPE =
            ContentResolver.CURSOR_DIR_BASE_TYPE +
                    "/vnd.intellibitz.android.intellibitzdb/all";
    public static final String MSGSGRPPEOPLECHATGROUPS_JOIN_DIR_MIME_TYPE =
            ContentResolver.CURSOR_DIR_BASE_TYPE +
                    "/vnd.intellibitz.android.intellibitzdb/join";
    public static final String MSGSGRPPEOPLECHATGROUPS_ITEM_MIME_TYPE =
            ContentResolver.CURSOR_ITEM_BASE_TYPE +
                    "/vnd.intellibitz.android.intellibitzdb/_id";
    public static final String MSGSGRPPEOPLECHATGROUPS_DATA_ITEM_MIME_TYPE =
            ContentResolver.CURSOR_ITEM_BASE_TYPE +
                    "/vnd.intellibitz.android.intellibitzdb/id";
    public static final String MSGSGRPPEOPLECHATGROUPS_JOIN_ITEM_MIME_TYPE =
            ContentResolver.CURSOR_ITEM_BASE_TYPE +
                    "/vnd.intellibitz.android.intellibitzdb/join/_id";
    public static final String MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_MIME_TYPE =
            ContentResolver.CURSOR_ITEM_BASE_TYPE +
                    "/vnd.intellibitz.android.intellibitzdb/join/id";
    public static final String MSGSGRPPEOPLECHATGROUPS_RAW_DIR_MIME_TYPE =
            ContentResolver.CURSOR_DIR_BASE_TYPE +
                    "/vnd.intellibitz.android.intellibitzdb/raw/all";
    public static final String AUTHORITY =
            "intellibitz.intellidroid.content.MsgsGrpPeopleChatGroupsContentProvider";
    public static final Uri CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + TABLE_MSGSGRPPEOPLECHATGROUPS);
    public static final Uri JOIN_CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + "join_" + TABLE_MSGSGRPPEOPLECHATGROUPS);
    //    to execute raw sql.. example select count(*)
    public static final Uri RAW_CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + "raw_" + TABLE_MSGSGRPPEOPLECHATGROUPS);
    // UriMatcher stuff
    private static final int MSGSGRPPEOPLECHATGROUPS_DIR_TYPE = 1;
    private static final int MSGSGRPPEOPLECHATGROUPS_JOIN_DIR_TYPE = 2;
    private static final int MSGSGRPPEOPLECHATGROUPS_ITEM_TYPE = 3;
    private static final int MSGSGRPPEOPLECHATGROUPS_DATA_ITEM_TYPE = 4;
    private static final int MSGSGRPPEOPLECHATGROUPS_JOIN_ITEM_TYPE = 5;
    private static final int MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_TYPE = 6;
    private static final int MSGSGRPPEOPLECHATGROUPS_RAW_DIR_TYPE = 7;
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        // to get definitions...
        matcher.addURI(AUTHORITY, TABLE_MSGSGRPPEOPLECHATGROUPS,
                MSGSGRPPEOPLECHATGROUPS_DIR_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGSGRPPEOPLECHATGROUPS,
                MSGSGRPPEOPLECHATGROUPS_JOIN_DIR_TYPE);
        matcher.addURI(AUTHORITY, TABLE_MSGSGRPPEOPLECHATGROUPS +
                "/#", MSGSGRPPEOPLECHATGROUPS_ITEM_TYPE);
        matcher.addURI(AUTHORITY, TABLE_MSGSGRPPEOPLECHATGROUPS +
                "/*", MSGSGRPPEOPLECHATGROUPS_DATA_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGSGRPPEOPLECHATGROUPS +
                        "/#", MSGSGRPPEOPLECHATGROUPS_JOIN_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "join_" + TABLE_MSGSGRPPEOPLECHATGROUPS +
                        "/*", MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_TYPE);
        matcher.addURI(AUTHORITY,
                "raw_" + TABLE_MSGSGRPPEOPLECHATGROUPS, MSGSGRPPEOPLECHATGROUPS_RAW_DIR_TYPE);
        // to get suggestions...
...
//        if (!bcc.isEmpty())
    ...
    public static long createOrUpdateMsgsGrpPeopleChatGroups(
...
*/
    ...
    public static void savesMsgsGrpPeopleChats(
        ...
        if (0 == did) {
            cursor = databaseHelper.query(db, TABLE_MSGSGRPPEOPLECHATGROUPS,
            ...
            cursor = databaseHelper.query(db, MsgsGrpPeopleChatGroupsContentProvider.TABLE_MSGSGRPPEOPLECHATGROUPS,
                    new String[]{MessageItemColumns.KEY_ID},
                    colName + " = ?",
                    new String[]{devId}, null);
        }
        if (null == cursor || 0 == cursor.getCount()) {
            ...
            databaseHelper.insert(db, MsgsGrpPeopleChatGroupsContentProvider.TABLE_MSGSGRPPEOPLECHATGROUPS, null, values);
        } else {
            cursor.close();
            databaseHelper.update(db, MsgsGrpPeopleChatGroupsContentProvider.TABLE_MSGSGRPPEOPLECHATGROUPS, values,
                    colName + " = ?",
                    new String[]{devId});
        }
    }
...
//                MainApplicationSingleton.getDateTimeMillis(messageItem.getTimestamp()));
    ...
    public String getType(@NonNull Uri uri) {
        switch (URI_MATCHER.match(uri)) {
            case MSGSGRPPEOPLECHATGROUPS_DIR_TYPE:
                return MSGSGRPPEOPLECHATGROUPS_DIR_MIME_TYPE;
            case MSGSGRPPEOPLECHATGROUPS_JOIN_DIR_TYPE:
                return MSGSGRPPEOPLECHATGROUPS_JOIN_DIR_MIME_TYPE;
            case MSGSGRPPEOPLECHATGROUPS_ITEM_TYPE:
                return MSGSGRPPEOPLECHATGROUPS_ITEM_MIME_TYPE;
            case MSGSGRPPEOPLECHATGROUPS_DATA_ITEM_TYPE:
                return MSGSGRPPEOPLECHATGROUPS_DATA_ITEM_MIME_TYPE;
            case MSGSGRPPEOPLECHATGROUPS_JOIN_ITEM_TYPE:
                return MSGSGRPPEOPLECHATGROUPS_JOIN_ITEM_MIME_TYPE;
            case MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_TYPE:
                return MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_MIME_TYPE;
            case MSGSGRPPEOPLECHATGROUPS_RAW_DIR_TYPE:
                return MSGSGRPPEOPLECHATGROUPS_RAW_DIR_MIME_TYPE;
            case SEARCH_SUGGEST:
...
//        String id = uri.getLastPathSegment();
        switch (URI_MATCHER.match(uri)) {
            ...
            case MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_TYPE:
                try {
                    cursor = databaseHelper.query(TABLE_MSGSGRPPEOPLECHATGROUPS,
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_TYPE:
                try {
                    long id = databaseHelper.insert(TABLE_MSGSGRPPEOPLECHATGROUPS, null, values);
    ...
    public int update(@NonNull Uri uri, ContentValues values, String where, String[] whereArgs) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_TYPE:
                try {
                    id = databaseHelper.update(TABLE_MSGSGRPPEOPLECHATGROUPS, values, where, whereArgs);
                    updateUri = ContentUris.withAppendedId(
                            MsgsGrpPeopleChatGroupsContentProvider.CONTENT_URI, id);
                    Context context = getContext();
                    if (null != context) {
            ...
            case MSGSGRPPEOPLECHATGROUPS_JOIN_DATA_ITEM_TYPE:
                try {
                    id = databaseHelper.delete(
                            TABLE_MSGSGRPPEOPLECHATGROUPS, where, whereArgs);
