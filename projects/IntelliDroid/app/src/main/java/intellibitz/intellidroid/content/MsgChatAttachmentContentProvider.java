 ...
 *
 */
public class MsgChatAttachmentContentProvider extends
        ContentProvider {

    public static final String TAG = "MsgChatAttachmentCP";
    public static final String TABLE_MSGCHATATTACHMENT = "msgchatattachment";
    public static final String CREATE_TABLE_MSGCHATATTACHMENT = "CREATE TABLE "
            + TABLE_MSGCHATATTACHMENT + MessageItemColumns.MESSAGECHAT_SCHEMA;
    ...
    public static String AUTHORITY =
            "intellibitz.intellidroid.content.MsgChatAttachmentContentProvider";
    public static final Uri CONTENT_URI = Uri.parse(
            SCHEME + AUTHORITY + "/" + TABLE_MSGCHATATTACHMENT);
    ...
    private static UriMatcher buildUriMatcher() {
        ...
        // to get definitions...
        matcher.addURI(AUTHORITY, TABLE_MSGCHATATTACHMENT,
                ATTACHMENT_DIR_TYPE);
        matcher.addURI(AUTHORITY, TABLE_MSGCHATATTACHMENT +
    ...
    public static ArrayList<MessageItem> fillAttachmentsFromCursor(Cursor cursor) {
        if (null == cursor || 0 == cursor.getCount()) return null;
        ArrayList<MessageItem> attachmentItems = new ArrayList<>(cursor.getCount());
        cursor.moveToFirst();
        do {
            MessageItem attachmentItem = new MessageItem();
            attachmentItem.set_id(cursor.getLong(cursor.getColumnIndex(MessageItemColumns.KEY_ID)));
            attachmentItem.setDataId(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_DATA_ID)));
            attachmentItem.setMsgAttachID(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_MSGATTCH_ID)));
            attachmentItem.setPartID(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_PARTID)));
            attachmentItem.setDocType(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_DOC_TYPE)));
            attachmentItem.setType(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_TYPE)));
            attachmentItem.setMessageType(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_MESSAGE_TYPE)));
            attachmentItem.setSubType(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_SUBTYPE)));
            attachmentItem.setName(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_NAME)));
            attachmentItem.setDescription(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_DESCRIPTION)));
            attachmentItem.setEncoding(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_ENCODING)));
            attachmentItem.setLanguage(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_LANGUAGE)));
            attachmentItem.setMd5(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_MD5)));
            attachmentItem.setDownloadURL(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_DOWNLOAD_URL)));
            attachmentItem.setProfilePic(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_PIC)));
            attachmentItem.setCloudPic(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_CLOUD_PIC)));
            attachmentItem.setSize(cursor.getInt(cursor.getColumnIndex(MessageItemColumns.KEY_SIZE)));
            attachmentItem.setTimestamp(cursor.getInt(cursor.getColumnIndex(MessageItemColumns.KEY_TIMESTAMP)));
            attachmentItem.setDateTime(cursor.getString(cursor.getColumnIndex(MessageItemColumns.KEY_DATETIME)));
    ...
    public static void setAttachmentItemFromJson(MessageItem attachmentMessage, JSONObject response) throws
        ...
        attachmentMessage.setMessageType(MessageItem.CHAT);
    }
    ...
    public static long[] createOrUpdateMessageAttachments(
        ...
        for (MessageItem attachment : items) {
            ids[i++] = createOrUpdateMessageAttachment(
                    databaseHelper, databaseHelper.getWritableDatabase(), attachment, id);
        }
        return ids;
    }
...
//        checks if the same email attachment is already present for the message thread
        ...
        Cursor cursor = databaseHelper.query(TABLE_MSGCHATATTACHMENT,
                new String[]{
...
//        creates the join
    ...
    public static Cursor getAllMessageAttachmentsCursor(
        ...
        String selectQuery = "SELECT  * FROM " + MessageChatContentProvider.TABLE_MESSAGECHAT + " msgs " +
...
//        Log.e(TAG, selectQuery);
    ...
    public static long updateMessageThreadMessageChatAttachmentURL(
        ...
        int row = databaseHelper.update(TABLE_MSGCHATATTACHMENT, values,
...
//        fetch the attachment and send the id back.. for content updates
    ...
    public static long getMessageAttachmentJoin(DatabaseHelper databaseHelper, long id, long fk) {
        long _id = 0;
        Cursor c = databaseHelper.query(MessageChatContentProvider.TABLE_MESSAGECHAT_ATTACHMENTS_JOIN, new String[]{IntellibitzItemColumns.KEY_ID},
    ...
    public static void asyncUpdateChatAttachmentFilePathInDB(
...
//            Intent intent = new Intent(context, RcvDocService.class);
            ...
            intent.setAction(MainApplicationSingleton.INTENT_ACTION_FETCH_CHAT_ATTACHMENT_CLOUD);
